...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        artists = {}
        films = dict([(film['pk'],film) for film in json.loads(serialize('json', filmffestival_models.Film.objects.all()))])
        film_images = collections.defaultdict(list)
        for image in json.loads(serialize('json', filmffestival_models.Image.objects.all())):
            image['fields']['image'] = 'https://s3.amazonaws.com/com.mykonosbiennale.static/' + image['fields']['image']
            film_images[image['fields']['film']].append(image)
        project_films = collections.defaultdict(list)
        for film in json.loads(serialize('json', filmffestival_models.Film.objects.all())):
            project_films[film['fields']['project']].append(film)
            film['fields']['poster'] = 'https://s3.amazonaws.com/com.mykonosbiennale.static/' + film['fields']['poster']
            film['images'] = film_images[film['pk']]
        festival_project = collections.defaultdict(list)
        for project in json.loads(serialize('json', festival_models.Project.objects.all())):
            festival_project[project['fields']['festival']].append(project)
            project['films'] = project_films[project['pk']]
        for festival in json.loads(serialize('json', festival_models.Festival.objects.all())):
            festival['projects'] = festival_project[festival['pk']]
            pprint.pprint(festival)
        with open('films.json','wb') as mb_json:
            json.dump(festival, mb_json)

                    # for i,film in enumerate(json.loads(serialize('json', filmffestival_models.Film.objects.all()))):
        #     pprint.pprint((i, film))
